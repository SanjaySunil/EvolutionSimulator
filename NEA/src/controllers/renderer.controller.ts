...
export default class RendererController {
  ...
  private draw(cell: GridCell, x: number, y: number, width: number, height: number, colour: string): void {
    ...
    this.ctx.fillRect(cell.coord.x * 15 + x, cell.coord.y * 15 + y, width, height);
  }

  /** Renders an organism with rotation degrees specified. */
  public render_organism_cell(cell, rotationDegrees) {
    const cellX = cell.coord.x * this.pixel_size;
    const cellY = cell.coord.y * this.pixel_size;

    this.ctx.save();

    /** Translate to the center of the cell. */
    this.ctx.translate(cellX + this.pixel_size / 2, cellY + this.pixel_size / 2);
  ...
  public draw_sub_shape(x, y, width, height, color) {
  ...
  public fill_cell(cell: GridCell): void {
    if (cell.is_highlighted) return this.highlight_cell(cell);
      ...
      if (cell.owner && cell.owner.alive) {
        let rotation = cell.owner!.direction.to_angle();
      ...
      } else if (cell.owner && cell.owner.alive == false) {
        this.render_food_cell(cell);
      }
    } else if (cell.state == CellStates.EMPTY) this.render_empty_cell(cell);
    else if (cell.state == CellStates.FOOD) this.render_food_cell(cell);
      ...
      this.ctx.fillRect(cell.coord.x * this.pixel_size, cell.coord.y * this.pixel_size, this.pixel_size, this.pixel_size);
    }
  }
